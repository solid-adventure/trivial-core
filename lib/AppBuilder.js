...
class AppBuilder {
  ...
  async build(inputManifest, manifest_api_obj) {
    this.buildStart = Date.now()
    this.pushToGitHub(inputManifest)
    await this._saveBundleToManifest(inputManifest, manifest_api_obj)
    await this._buildLamda(inputManifest)
    await this._logBuildCompletion()
  }

  async formDataForManifestBundle(inputManifest) {
    const archiveBuffer = await this.createDownload(inputManifest)
    const formData = new FormData()
      formData.append('bundle', archiveBuffer, {
      name: 'bundle',
      filename: `bundle.zip`
    })
    return formData
  ...
  // NOTE: Early trivial-ui versions do not send the manifest_api_obj this feature relies on.
  //       This will fail silently for those applications.
  async _saveBundleToManifest(inputManifest, manifest_api_obj) {
    console.log(`_saveBundleToManifest] Starting`)
    if (!manifest_api_obj || typeof(manifest_api_obj.id) == 'undefined') { return }
    try {
      const api = new APIService('trivial', this.options.req)
      const data = await api.fetchJSON(`/manifests/${manifest_api_obj.id}`,
        {
          method: 'PUT',
          body: await this.formDataForManifestBundle(inputManifest)
        }
      )
      return data
    } catch (e) {
      console.log(`[_saveBundleToManifest] failed, ${e}`) // Temp Remove
    }
  }
