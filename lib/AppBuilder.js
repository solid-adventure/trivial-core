...
class AppBuilder {
  ...
  async getApiKey() {
    ...
    const res =  await api.fetchJSON(`/apps/${this.appId}/api_key/app_key`, {
      method: 'POST',
  ...
  async writeLocally(inputManifest) {
    ...
    let path = `./slugs/${inputManifest.app_id}`
    await this.getApiKey()
  ...
  _env(manifest) {
    return {
      ...
      INTERNAL_URL: `${manifest.log_to.host}:${manifest.log_to.port}`,
  ...
  async _updateLambda(manifest, writer, layers, client, info) {
    ...
    await this._ensureCorrectLayers(client, info, layers, manifest)
    await this._updateEnvironment(client, manifest)
  ...
  async _updateEnvironment(client, manifest) {
    this.logger.debug(`[AppBuilder][_updateEnvironment] updating environment`)
    await client.send(
