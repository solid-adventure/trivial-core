...
class AppBuilder {
  ...
  async writeLocally(inputManifest) {
    this.logger.debug(`[AppBuilder][writeLocally] writing ${inputManifest.app_id}`)
    let path = `./slugs/${inputManifest.app_id}`
    fs.mkdir(path, {recursive: true})
    const writer = new FileWriter(path)
    await this._assembleCode(inputManifest, true, writer)
    this.logger.debug(`[AppBuilder][writeLocally] finished writing ${inputManifest.app_id}`)
  }

  async _assembleCode(inputManifest, useEnv, writer) {
    const manifest = await this._normalizeManifest(inputManifest)
    this.logger.debug(`[AppBuilder][build] building ${manifest.app_id}`)
    ...
    this.logger.debug(`[AppBuilder][build] build complete`)
