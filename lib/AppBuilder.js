...
class AppBuilder {
  ...
  github_attrs(inputManifest) {
    return [
      ...
      'Test Suite', // Author name // TODO Dynamic,
      'test@withtrivial.com' // Author email // TODO Dynamic
    ]
  }

  async build(inputManifest) {
    console.log(`[AppBuilder] Local`)

    if (inputManifest.github ) {
      console.log(`[AppBuilder]  Pushing to github_repo`)
      ...
      await gitManager.pushChanges(message)
      console.log(`[AppBuilder] Done`)
    }

    if (inputManifest.deploy_via === "github") {
      console.log(`[AppBuilder] Done, will deploy via git`)
      return
    }

    // No deploy_via set, attempt to deploy as Lambda
    try {
  ...
  async _assembleCode(inputManifest, useEnv, writer) {
    ...
    const template = new AppTemplate(manifest.template, manifest.version)
    await this._writeTemplate(manifest, writer, template)

    await this._writeStaticFiles(manifest, writer)
    ...
    this.logger.debug(`[AppBuilder]    Done!`)
  ...
  async _writeStaticFiles(manifest, writer) {
