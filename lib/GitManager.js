...
class GitManager {
	...
	async pushDirectory() {
		try {
  		...
  		const message = `My commit message` // TODO Dynamic
		  const newCommit = await this.createNewCommit(message, newTree.sha, currentCommit.commitSha)
		  console.log(`newCommit.sha: ${newCommit.sha}`)

			// 5. Set the branch ref to the new commit
			const data = await this.setBranchToCommit(this.branch, newCommit.sha)
			console.log(data)
	...
	async createNewCommit(message, currentTreeSha, currentCommitSha) {
      ...
      author: {
        name: "James Marks", // TODO Dynamic
        email: "james.marks@withtrivial.com" // TODO Dynamic
      }
	  })
	  return data
	}
