...
class CodeGenerator {
  ...
  async build(writer) {
    ...
    console.log(`[CodeGenerator] Done`)
  }
  ...
  // manifest.app_entry_module = 'Thin' will call _buildConversionModuleThin
  async _buildConversionModule(writer) {
    if (!this.manifest.app_entry_module) {
      return this._buildConversionModuleOrig(writer)
    } 
    try {
      const conversionModule = `_buildConversionModule${this.manifest.app_entry_module}`
      return this[conversionModule](writer)
    } catch (e) {
      console.log(`Could not build app_entry_module for ${this.manifest.app_entry_module}: ${e}`)
    }
  }

  // Returns the output of the last step in the application
  async _buildConversionModuleThin(writer) {
    await writer.write('Application.js',
`
const { tryRequire, lastLoadErrorFor } = require('./utils')
const ActionBase = require('./lib/actionsv1/ActionBase')
...
// Returns {topic: null, status: 200}
  async _buildConversionModuleOrig(writer) {
